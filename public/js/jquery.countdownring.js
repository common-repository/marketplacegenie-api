...
;(function($) {
	...
	var countdown = function(elem, time_value, total_time, onSuccss, options   ) {
			...
			options = $.extend(true, {
				...
				mark:  {color: "#2276CB", width: 1}, //mask stroke
		...
		var start = function () {
            ...
            if(options.xstoptime == time_value ){  return;;}
            if(time_value > 180 && time_value < 360  ){ options.outer='{color: "orange", width: 10}' ; }
            if(time_value > 180 && time_value < 180  ){ options.outer='{color: "blue", width: 10}';}
